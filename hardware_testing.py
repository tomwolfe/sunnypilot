...
class HardwareStressTester:
    ...
    def _collect_hardware_data(self) -> Dict[str, Any]:
        ...
        # Estimate power draw based on CPU and RAM usage
        estimated_power = self._estimate_power(overall_cpu, ram_used_mb)
    ...
    def _estimate_power(self, cpu_percent: float, ram_mb: float) -> float:
        ...
        # For ARM CPU, max power is typically lower than x86
        cpu_power_factor = 0.1  # adjusted for ARM architecture
        cpu_component = min(6.0, (cpu_percent / 100.0) * 7.0)  # max ~7W at 100% CPU
        
        # RAM power component
        ram_power_factor = 0.0001  # additional watts per MB
        ram_component = (ram_mb / self.constraints.ram_limit_mb) * 0.5  # max additional ~0.5W
        
        estimated_power = base_power + cpu_component + ram_component
        
        # Cap at 10W budget to represent the power constraint
        return min(estimated_power, self.constraints.power_budget_w)
