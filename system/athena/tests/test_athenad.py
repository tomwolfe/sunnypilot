...
class TestAthenadMethods:
  ...
  def test_do_upload(self, host, compress):
    ...
    item = athenad.UploadItem(path=upload_fn, url="http://localhost:1238", headers={}, created_at=int(datetime.now(UTC).timestamp() * 1000), id='')
    with pytest.raises(requests.exceptions.ConnectionError):
      athenad._do_upload(item)

    item = athenad.UploadItem(path=upload_fn, url=f"{host}/qlog.zst", headers={}, created_at=int(datetime.now(UTC).timestamp() * 1000), id='')
  ...
  def test_upload_handler(self, host):
    fn = self._create_file('qlog.zst')
    item = athenad.UploadItem(path=fn, url=f"{host}/qlog.zst", headers={}, created_at=int(datetime.now(UTC).timestamp() * 1000), id='', allow_cellular=True)
  ...
  def test_upload_handler_timeout(self):
    ...
    item = athenad.UploadItem(
        path=fn,
        url="http://localhost:44444/qlog.zst",
        headers={},
  ...
  def test_upload_queue_persistence(self):
    item1 = athenad.UploadItem(path="_", url="_", headers={}, created_at=int(datetime.now(UTC).timestamp()), id='id1')
    item2 = athenad.UploadItem(path="_", url="_", headers={}, created_at=int(datetime.now(UTC).timestamp()), id='id2')
