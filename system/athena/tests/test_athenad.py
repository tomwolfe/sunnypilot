...
class TestAthenadMethods:
  ...
  def _wait_for_upload():
    # Wait for the upload handler to start processing (pick up an item)
    # Then wait for the processing to complete, which may result in
    # the item being removed (success) or put back (retry)
    start_time = time.monotonic()
    item_picked_up = False

    # Wait up to 5 seconds for an item to be picked up for processing
    ...
    # Wait additional time for the operation to complete (success or retry)
    if item_picked_up:
      time.sleep(0.75)  # Wait for operation to complete
    else:
      time.sleep(0.5)  # If no item was picked up, still wait briefly
  ...
  def test_do_upload(self, host, compress):
    ...
    item = athenad.UploadItem(path=upload_fn, url="http://localhost:1238", headers={}, created_at=int(datetime.now(timezone.utc).timestamp() * 1000), id='')
    with pytest.raises(requests.exceptions.ConnectionError):
      athenad._do_upload(item)

    item = athenad.UploadItem(path=upload_fn, url=f"{host}/qlog.zst", headers={}, created_at=int(datetime.now(timezone.utc).timestamp() * 1000), id='')
  ...
  def test_upload_handler_timeout(self):
    ...
    fn = self._create_file('qlog.zst')
    item = athenad.UploadItem(path=fn, url="http://localhost:44444/qlog.zst", headers={}, created_at=int(datetime.now(timezone.utc).timestamp() * 1000), id='', allow_cellular=True)
  ...
  def test_cancel_upload(self):
    item = athenad.UploadItem(
      path="qlog.zst", url="http://localhost:44444/qlog.zst", headers={}, created_at=int(datetime.now(timezone.utc).timestamp() * 1000), id='id', allow_cellular=True
    )
    athenad.upload_queue.put_nowait(item)
  ...
  def test_upload_queue_persistence(self):
    item1 = athenad.UploadItem(path="_", url="_", headers={}, created_at=int(datetime.now(timezone.utc).timestamp()), id='id1')
    item2 = athenad.UploadItem(path="_", url="_", headers={}, created_at=int(datetime.now(timezone.utc).timestamp()), id='id2')
