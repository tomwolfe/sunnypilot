...
ALL_SENSORS = {
  Sensor.lsm6ds3: {
    ...
    SensorConfig("gyroUncalibrated", 0, 0.2),
...
def read_sensor_events(duration_sec):
  sensor_types = ['accelerometer', 'gyroscope', 'magnetometer', 'accelerometer2', 'gyroscope2', 'temperatureSensor', 'temperatureSensor2']
  socks = {}
...
class TestSensord:
  ...
  def test_lsm6ds3_timing(self, subtests):
    ...
    for s, vals in sensor_t.items():
      with subtests.test(sensor=s):
        ...
        high_delay_diffs = list(filter(lambda d: d >= 20.0, tdiffs))
        assert len(high_delay_diffs) < 15, f"Too many large diffs: {high_delay_diffs}"

        avg_diff = sum(tdiffs) / len(tdiffs)
        avg_freq = 1.0 / (avg_diff * 1e-3)
        assert 92.0 < avg_freq < 114.0, f"avg freq {avg_freq}Hz wrong, expected 104Hz"
  ...
  def test_logmonottime_timestamp_diff(self):
    ...
    high_delay_diffs = set(filter(lambda d: d >= 25.0, tdiffs))
