...
class TestMonitoring:
  ...
  def test_long_traffic_light_victim(self):
    ...
    assert (
      events[int((d_status.settings._DISTRACTED_TIME - d_status.settings._DISTRACTED_PRE_TIME_TILL_TERMINAL + 1) / DT_DMON)].names[0]
      == EventName.preDriverDistracted
    )
  ...
  def test_somehow_indecisive_model(self):
    ...
    assert (
      EventName.preDriverUnresponsive
      in events[int((INVISIBLE_SECONDS_TO_ORANGE - 1 + DT_DMON * d_status.settings._HI_STD_FALLBACK_TIME - 0.1) / DT_DMON)].names
    )
    assert (
      EventName.promptDriverUnresponsive
      in events[int((INVISIBLE_SECONDS_TO_ORANGE - 1 + DT_DMON * d_status.settings._HI_STD_FALLBACK_TIME + 0.1) / DT_DMON)].names
    )
    assert EventName.driverUnresponsive in events[int((INVISIBLE_SECONDS_TO_RED - 1 + DT_DMON * d_status.settings._HI_STD_FALLBACK_TIME + 0.1) / DT_DMON)].names
