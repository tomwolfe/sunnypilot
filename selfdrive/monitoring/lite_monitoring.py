...
class LightweightSafetyChecker:
  ...
  def __init__(self, CP=None):
    ...
    self.safety_thresholds = (
      self._get_vehicle_specific_thresholds(CP)
      if CP
      else {
  ...
  def validate_outputs(self, actuators, car_state, radar_state) -> dict:
    ...
    if hasattr(radar_state, 'leadOne') and radar_state.leadOne.status and car_state.vEgo > 0.1:
    ...
    if not safety_report['safe']:
      if 'steering_angle_limit_exceeded' in violations_list or 'steering_rate_limit_exceeded' in violations_list:
        safety_report['recommended_action'] = 'disengage'
      elif 'long_accel_limit_exceeded' in violations_list or 'lat_accel_limit_exceeded' in violations_list or 'forward_collision_imminent' in violations_list:
        safety_report['recommended_action'] = 'decelerate'
