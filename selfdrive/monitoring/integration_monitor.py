  ...
  def update_integration(self) -> Dict[str, Any]:
    ...
    if self.sla_controller:
      try:
        ...
        v_cruise_cluster = car_state.vCruise if hasattr(car_state, 'vCruise') else 255.0
        ...
        from openpilot.sunnypilot.selfdrived.events import EventsSP
        events_sp = EventsSP()
    ...
    if 'avg_lateral_jerk' in health_report:
      # Lower jerk = higher stability
      jerk_factor = max(0.0, 1.0 - health_report['avg_lateral_jerk'] / 10.0)  # Normalize
