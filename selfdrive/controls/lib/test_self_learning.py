...
def test_safety_curvature_validation(self_learning_safety):
  ...
  safe_curvature, is_safe = safety.validate_curvature_adjustment(original_curvature, unsafe_curvature, v_ego=20.0)
...
def test_safety_parameter_validation(self_learning_safety):
  ...
  safe_value, is_safe = safety.validate_parameter_adjustment('lateral_control_factor', current_value, proposed_value, v_ego=15.0)
...
def test_performance(safe_self_learning_manager):
  ...
  for _i in range(1000):
    ...
    CS.vEgo = 15.0
    CS.steeringTorque = 0.1
    learning_manager.update(CS, desired_curvature=0.05, actual_curvature=0.049, steering_torque=0.1, v_ego=CS.vEgo)
