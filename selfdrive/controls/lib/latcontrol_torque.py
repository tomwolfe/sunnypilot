...
class LatControlTorque(LatControl):
  ...
  def update(
    ...
    desired_curvature,  # Desired path curvature in 1/meter
    calibrated_pose,
    curvature_limited,
    lat_delay,
    adaptive_gains: dict,
    ...
    if not active:
      ...
      if hasattr(self.extension, '_nnlc_enabled') and getattr(self.extension, '_nnlc_enabled', False):
        ...
        pid_log.saturated = bool(
          self._check_saturation(curvature_limited or abs(output_torque) >= self.steer_max - 1e-3, CS, steer_limited_by_safety, curvature_limited)
        )
      else:
