...
class SelfLearningManager:
  ...
  def update_from_driver_intervention(
    self,
    CS,
    ...
    if should_learn:
      ...
      if self.learning_samples % 25 == 0:  # Log every 25 learning events
        ...
        for key in self.adaptive_params:
          ...
          param_changes[key] = {'from': original_value, 'to': new_value, 'change': change}
        cloudlog.info(
  ...
  def _update_learning_context(self, CS, v_ego: float, desired_curvature: float):
    ...
    if hasattr(CS, 'wheelSpeeds') and CS.wheelSpeeds is not None and abs(v_ego) > 1.0:
      try:
        ...
        if (
          hasattr(CS, 'brakePressed') and CS.brakePressed and v_ego > 5.0 and abs(CS.aEgo) > 3.0 and abs(CS.vEgo) > 5.0
        ):  # Hard braking with high deceleration at meaningful speed
          # Check for wheel slip by comparing vehicle speed to wheel speeds
          if hasattr(CS.wheelSpeeds, 'fl') and hasattr(CS.wheelSpeeds, 'fr') and CS.wheelSpeeds.fl is not None and CS.wheelSpeeds.fr is not None:
  ...
  def update_from_model_accuracy(self, desired_curvature: float, actual_curvature: float, v_ego: float, model_confidence: float = 1.0) -> None:
    """
  ...
  def periodic_update(self, CS=None):
    ...
    if self.enhanced_monitor:
      ...
      over_adaptation_result = self.enhanced_monitor.monitor_over_adaptation(self.adaptive_params, self.learning_context)
    ...
    if self.enhanced_monitor:
      start_time = time.monotonic()
      self.enhanced_monitor.track_computational_performance(start_time)
  ...
  def _comprehensive_monitoring(self):
    ...
    if len(self.model_accuracy_history) > 0:
      ...
      if recent_accuracy:
        ...
        monitoring_data['performance_metrics']['error_trend'] = np.mean(errors[-10:]) - np.mean(errors[:10]) if len(errors) > 20 else 0.0
    ...
    if 'learning_efficiency' in monitoring_data:
      ...
      if eff > 0.8:  # Very high learning frequency might indicate instability
        cloudlog.warning(
          "Self-Learning High Activity Alert - " + f"Learning Efficiency: {eff:.3f}, " + "Learning may be too frequent, " + "consider adjusting thresholds"
        )
  ...
  def _regularize_parameters(self, over_adaptation=False):
    ...
    if abs(factor - 1.0) > 0.02:  # Lower threshold for regularization (2% instead of 5%)
      ...
      self.adaptive_params['lateral_control_factor'] = regularization_factor * factor + (1 - regularization_factor) * 1.0
    ...
    if abs(accel_factor - 1.0) > 0.02:  # Lower threshold for regularization (2% instead of 5%)
      if abs(accel_factor - 1.0) > 0.25:  # More than 25% deviation - aggressive regularization
        ...
        regularization_factor = slow_regularization_factor
      self.adaptive_params['acceleration_factor'] = regularization_factor * accel_factor + (1 - regularization_factor) * 1.0
    ...
    if abs(reaction_time - 0.2) > 0.05:  # Regularize if significantly different from default
      ...
      self.adaptive_params['reaction_time_compensation'] = reg_factor * reaction_time + (1 - reg_factor) * 0.2
  ...
  def validate_learned_parameters_safety(self, CS, desired_curvature: float, desired_acceleration: float, v_ego: float) -> dict:
    ...
    if self.enhanced_safety_validator:
      enhanced_validation = self.enhanced_safety_validator.validate_with_computational_efficiency(self.adaptive_params, v_ego)
    ...
    if hasattr(self, '_prev_validation_curvatures'):
      if len(self._prev_validation_curvatures) >= 5:
        recent_changes = [
          abs(self._prev_validation_curvatures[i] - self._prev_validation_curvatures[i - 1]) for i in range(1, len(self._prev_validation_curvatures))
        ]
    ...
    total_param_drift = sum(
      abs(self.adaptive_params[p] - 1.0)
      if p not in ['curvature_bias']
    ...
    if critical_violations_list:
      cloudlog.error(
        "CRITICAL SELF-LEARNING SAFETY VIOLATIONS - "
      ...
      cloudlog.warning(
        "Self-Learning Safety Validation - "
        + f"Safe: {validation_results['is_safe']}, "
