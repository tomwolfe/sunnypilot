...
class SelfLearningSafety:
  ...
  def validate_curvature_adjustment(self, original_curvature: float, adjusted_curvature: float, v_ego: float) -> tuple[float, bool]:
    """
    ...
    clamped_curvature = np.clip(adjusted_curvature, self.min_curvature, self.max_curvature)
    # Also limit based on speed-dependent safety
    clamped_curvature = np.clip(clamped_curvature, -max_safe_curvature_for_speed, max_safe_curvature_for_speed)
    ...
    if time_diff > 0:
      ...
      if curvature_rate > max_curvature_rate:
        ...
        clamped_curvature = self.prev_curvature + np.clip(curvature_diff, -max_curvature_change, max_curvature_change)
  ...
  def validate_parameter_adjustment(self, param_name: str, current_value: float, proposed_value: float, v_ego: float) -> tuple[float, bool]:
    """
  ...
  def update_safety_score(self, CS: car.CarState, model_outputs: dict, adjusted_outputs: dict) -> float:
    """
    ...
    if 'desired_acceleration' in model_outputs and 'desired_acceleration' in adjusted_outputs:
      ...
      if abs_accel_change > 0.5:  # Large acceleration change threshold
        ...
        # Higher score if the change is small (more stable)
        accel_change_score = max(0.5, 1.0 - abs_accel_change)  # At least 0.5 for stability
      scores['acceleration_stability'] = accel_change_score
    ...
    base_weights = {
      ...
      'acceleration_stability': 0.15,  # Important for acceleration stability
      'speed': 0.1,  # Important for speed-appropriate behavior
...
class SafeSelfLearningManager:
  ...
  def update(
    ...
    if self._interaction_monitor_counter % 200 == 0:
      ...
      cloudlog.info(
        "Learning Interaction Monitor - Lateral Factor: {lateral_factor:.3f}, "
  ...
  def adjust_curvature(self, original_curvature: float, v_ego: float) -> float:
    ...
    # Validate for safety
    safe_curvature, is_safe = self.safety.validate_curvature_adjustment(original_curvature, adjusted_curvature, v_ego)
    if not is_safe:
  ...
  def adjust_acceleration(self, original_accel: float, v_ego: float) -> float:
    ...
    # Validate for safety
    safe_accel, is_safe = self.safety.validate_acceleration_adjustment(original_accel, adjusted_accel)
    if not is_safe:
      # Revert to original if adjustment is unsafe
  ...
  def adjust_curvature_with_safety_validation(self, original_curvature: float, v_ego: float, CS) -> float:
    ...
    validation_results = self.validate_learned_parameters_safety(
      ...
      adjusted_curvature,
      0.0,
      v_ego,  # acceleration will be validated separately
    )
  ...
  def adjust_acceleration_with_safety_validation(self, original_accel: float, v_ego: float, CS) -> float:
    ...
    validation_results = self.validate_learned_parameters_safety(
      ...
      0.0,
      adjusted_accel,
      v_ego,  # curvature will be validated separately
    )
    # Log safety validation results if issues found
    if not validation_results['is_safe'] or validation_results['confidence_in_safety'] < 0.7:
