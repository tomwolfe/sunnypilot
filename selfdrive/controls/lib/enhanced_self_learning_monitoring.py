...
class EnhancedSelfLearningMonitor:
  ...
  def monitor_over_adaptation(self, adaptive_params: dict[str, float], learning_context: dict) -> dict:
    ...
    context_record = {'timestamp': time.monotonic(), 'context': learning_context.copy(), 'params': adaptive_params.copy()}
        ...
        for param_name, curr_value in curr_params.items():
          ...
          if param_name == 'lateral_control_factor':
            ...
            if drift_rate > 0.05:  # Very sensitive parameter, low threshold
              result['critical_violations'].append(f'CRITICAL: Large {param_name} change: {drift_rate:.3f}')
              result['over_adaptation_detected'] = True
              result['triggering_conditions'].append(f'Critical {param_name} change: {drift_rate:.3f}')
              result['triggering_conditions'].append(f'Excessive {param_name} changes: {drift_rate:.3f}')
    ...
    if result['critical_violations']:
      cloudlog.error(f"CRITICAL OVER-ADAPTATION VIOLATIONS: {', '.join(result['critical_violations'])}")
  ...
  def track_computational_performance(self, start_time: float) -> float:
    ...
    if computation_time > self.critical_time_threshold:
      ...
      self.safety_violations.append(
        {'timestamp': time.monotonic(), 'type': 'performance', 'value': computation_time, 'threshold': self.critical_time_threshold}
      )
    ...
    if len(self.computation_times) >= 10:  # Need minimum samples for statistics
      ...
      if len(self.computation_times) >= 50 and recent_avg > avg_time * 1.5:
        cloudlog.warning(f"Performance degradation detected: Recent avg {recent_avg * 1000:.2f}ms vs historical avg {avg_time * 1000:.2f}ms")
        # Add to safety violations for tracking
        self.safety_violations.append({'timestamp': time.monotonic(), 'type': 'performance_degradation', 'value': recent_avg, 'baseline': avg_time})
      # Log performance statistics periodically but more selectively
      if len(self.computation_times) % 100 == 0:
        cloudlog.info(
          f"Performance Stats - Avg: {avg_time * 1000:.2f}ms, "
        ...
        if avg_time > self.max_computation_time * 1.1:  # 10% over target
          cloudlog.error(f"Average computational overhead exceeds target by {(avg_time / self.max_computation_time - 1) * 100:.1f}%")
  ...
  def update_vehicle_calibration(self, v_ego: float, curvature: float, lateral_accel: float):
    ...
    if abs(lateral_accel) < self.vehicle_calibration['lateral_acceleration_limit'] * 0.8:
      ...
      self.vehicle_calibration['lateral_acceleration_limit'] = max(
        1.5,
        min(5.0, self.vehicle_calibration['lateral_acceleration_limit']),  # Reduced upper bound for safety
      )
...
class EnhancedSafetyValidator:
  ...
  def validate_with_computational_efficiency(self, params: dict, v_ego: float) -> dict:
    ...
    self.param_validation_history.append({'timestamp': time.monotonic(), 'params': params.copy(), 'results': results.copy()})
    ...
    if critical_violations_list:
      self.critical_safety_violations.append({'timestamp': time.monotonic(), 'violations': critical_violations_list, 'params': params.copy()})
