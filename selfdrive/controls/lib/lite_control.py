...
class LightweightAdaptiveGainScheduler:
  ...
  def get_adaptive_gains(self, v_ego: float, thermal_state: float) -> dict:
    ...
    long_speed_factor = max(
      ...
      min(self.vehicle_tuning['max_long_gain_multiplier'], 1.0 + (v_ego / self.vehicle_tuning['long_speed_gain_factor'])),
    )
    ...
    lat_speed_factor = max(
      ...
      min(self.vehicle_tuning['max_lat_gain_multiplier'], 1.0 + (v_ego / self.vehicle_tuning['lat_speed_gain_factor'])),
    )
    ...
    # For longitudinal:
    base_long_kp = self._get_base_gain(
      v_ego, getattr(self.CP.longitudinalTuning, 'kpBP', None), getattr(self.CP.longitudinalTuning, 'kpV', []), 1.0, "longitudinal kpV"
    )
    base_long_ki = self._get_base_gain(
      v_ego, getattr(self.CP.longitudinalTuning, 'kiBP', None), getattr(self.CP.longitudinalTuning, 'kiV', []), 0.1, "longitudinal kiV"
    )
    base_long_kf = getattr(self.CP.longitudinalTuning, 'kf', 0.0)

    # For lateral:
    base_lat_kp = self._get_base_gain(
      v_ego, getattr(self.CP.lateralTuning.pid, 'kpBP', None), getattr(self.CP.lateralTuning.pid, 'kpV', []), 0.5, "lateral kpV"
    )
    base_lat_ki = self._get_base_gain(
      v_ego, getattr(self.CP.lateralTuning.pid, 'kiBP', None), getattr(self.CP.lateralTuning.pid, 'kiV', []), 0.05, "lateral kiV"
    )
    base_lat_kd = 0.0
    ...
    if hasattr(self.CP.lateralTuning.pid, 'kdV') and getattr(self.CP.lateralTuning.pid, 'kdV', None):
      base_lat_kd = self._get_base_gain(
        v_ego, getattr(self.CP.lateralTuning.pid, 'kdBP', None), getattr(self.CP.lateralTuning.pid, 'kdV', []), 0.0, "lateral kdV"
      )
  ...
  def _get_base_gain(self, v_ego: float, bp_array: Optional[list[float]], gain_array: list[float], default_gain: float, gain_name: str) -> float:
    ...
    if (
      bp_array is not None
      and hasattr(bp_array, '__getitem__')
      and len(bp_array) > 0
      and hasattr(gain_array, '__getitem__')
      and len(gain_array) > 0
      and len(bp_array) == len(gain_array)
    ):
