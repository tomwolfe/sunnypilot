...
def run_mpc(lat_mpc=None, v_ref=30.0, x_init=0.0, y_init=0.0, psi_init=0.0, curvature_init=0.0, lane_width=3.6, poly_shift=0.0):
  if lat_mpc is None:
    lat_mpc = LateralMpc()
  lat_mpc.set_weights(1.0, 0.1, 0.0, 0.05, 800)
  ...
  p = np.column_stack([v_ref * np.ones(LAT_MPC_N + 1), CAR_ROTATION_RADIUS * np.ones(LAT_MPC_N + 1)])
