...
class TestLightweightSafetyChecker:
  ...
  def test_steering_rate_no_violation(self, safety_checker, mock_actuators, mock_car_state, mock_radar_state, mocker):
  ...
  def test_multiple_violations_steering_rate_and_collision(self, safety_checker, mocker):
    mock_actuators = Mock()
    mock_actuators.accel = 0.0
  ...
  def test_trigger_fail_safe_disengage(self, safety_checker, mock_car_state):
    safety_report = {'safe': False, 'violations': ['steering_angle_limit_exceeded'], 'recommended_action': 'disengage'}
  ...
  def test_trigger_fail_safe_decelerate(self, safety_checker, mock_car_state):
    safety_report = {'safe': False, 'violations': ['long_accel_limit_exceeded'], 'recommended_action': 'decelerate'}
  ...
  def test_trigger_fail_safe_continue(self, safety_checker, mock_car_state):
    safety_report = {'safe': True, 'violations': [], 'recommended_action': 'continue'}
    fail_safe_cmd = safety_checker.trigger_fail_safe(safety_report, mock_car_state)
    assert fail_safe_cmd['enabled'] is True
