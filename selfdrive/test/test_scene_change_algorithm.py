...
class TestSceneChangeAlgorithm:
  ...
  def test_identical_frames_skip(self):
    ...
    assert should_run == False, "Identical frames should be skipped"
    assert scene_changed == False, "Identical frames should not be marked as changed"
  ...
  def test_different_frames_run(self):
    ...
    assert should_run == True, "Different frames should trigger model run"
    assert scene_changed == True, "Different frames should be marked as changed"
  ...
  def test_frame_skip_limit(self):
    ...
    assert should_run == True, "Should run model when max skip reached"
  ...
  def test_moderately_different_frames(self):
    ...
    assert should_run == False, "Moderately different frames should be skipped"
    assert scene_changed == False, "Moderately different frames should not be marked as changed"
  ...
  def test_different_shape_frames(self):
    ...
    assert should_run == True, "Different shaped frames should trigger model run"
    assert scene_changed == True, "Different shaped frames should be marked as changed"
  ...
  def test_algorithm_sensitivity(self):
    ...
    if expected_diff > 3.0:
      assert should_run == True, f"Expected run due to diff {expected_diff} > 3.0"
    else:
      assert should_run == False, f"Expected skip due to diff {expected_diff} <= 3.0"
