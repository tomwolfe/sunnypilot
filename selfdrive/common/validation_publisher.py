...
class ValidationMetricsPublisher:
    ...
    def publish_metrics(self, enhanced_validation_result: Dict[str, Any], 
        ...
        try:
            ...
            # Set standard validation metrics
            validation_metrics.leadConfidenceAvg = enhanced_validation_result.get('base_confidence', 0.0)
            validation_metrics.leadConfidenceMax = enhanced_validation_result.get('situation_adjusted_confidence', 0.0)
            validation_metrics.laneConfidenceAvg = enhanced_validation_result.get('base_confidence', 0.0)
            validation_metrics.overallConfidence = enhanced_validation_result.get('situation_adjusted_confidence', 0.0)
            validation_metrics.isValid = enhanced_validation_result.get('system_safe', True)
            validation_metrics.confidenceThreshold = 0.6  # Standard threshold
            
            # Set enhanced metrics
            validation_metrics.enhanced.situationFactor = enhanced_validation_result.get('situation_factor', 1.0)
            validation_metrics.enhanced.speedAdjustedConfidence = enhanced_validation_result.get('speed_adjusted_confidence', 0.0)
            validation_metrics.enhanced.temporalConsistency = enhanced_validation_result.get('temporal_consistency', 1.0)
            validation_metrics.enhanced.systemSafe = enhanced_validation_result.get('system_safe', False)
            
            # Additional metrics for specific use cases
            validation_metrics.enhanced.leadConfidenceOk = enhanced_validation_result.get('lead_confidence_ok', True)
            validation_metrics.enhanced.laneConfidenceOk = enhanced_validation_result.get('lane_confidence_ok', True)
            validation_metrics.enhanced.overallConfidenceOk = enhanced_validation_result.get('overall_confidence_ok', True)
            validation_metrics.enhanced.laneChangeSafe = enhanced_validation_result.get('lane_change_safe', True)
            validation_metrics.enhanced.systemEngagementSafe = enhanced_validation_result.get('system_engagement_safe', True)
