import numpy as np
from selfdrive.modeld.parse_model_outputs import Parser, TemporalConsistencyFilter


def test_temporal_consistency_filter():
  """Test that the temporal consistency filter smooths model outputs over time."""
  filter = TemporalConsistencyFilter(buffer_size=5)
  ...
  smoothed_plan_1 = filter.smooth_plan(test_plan)
  smoothed_plan_2 = filter.smooth_plan(test_plan + np.random.rand(*test_plan.shape) * 0.1)  # Small perturbation
