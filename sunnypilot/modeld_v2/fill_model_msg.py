...
def get_curvature_from_output(output, vego, lat_action_t, mlsim):
  ...
  return float(
    get_curvature_from_plan(
      plan_output[:, Plan.T_FROM_CURRENT_EULER][:, 2], plan_output[:, Plan.ORIENTATION_RATE][:, 2], ModelConstants.T_IDXS, vego, lat_action_t
    )
  )
...
def fill_model_msg(
  base_msg: capnp._DynamicStructBuilder,
  extended_msg: capnp._DynamicStructBuilder,
  net_output_data: dict[str, np.ndarray],
  action: log.ModelDataV2.Action,
  publish_state: PublishState,
  vipc_frame_id: int,
  vipc_frame_id_extra: int,
  frame_id: int,
  frame_drop: float,
  timestamp_eof: int,
  ...
  hard_brake_predicted = (publish_state.prev_brake_5ms2_probs > ModelConstants.FCW_THRESHOLDS_5MS2).all() and (
    publish_state.prev_brake_3ms2_probs > ModelConstants.FCW_THRESHOLDS_3MS2
  ).all()
...
def fill_pose_msg(
  msg: capnp._DynamicStructBuilder,
  net_output_data: dict[str, np.ndarray],
  vipc_frame_id: int,
  vipc_dropped_frames: int,
  timestamp_eof: int,
  live_calib_seen: bool,
) -> None:
